...
def display(event):
    ...
    if (event.widget == canvas1 or event.widget == canvas2 or event.widget == canvas3 or event.widget == canvas4):
        ...
        if (event.widget == canvas1 ):
            img = dis_canvas.create_image(0.05*w_win, 0.45*h_win, anchor='w',image=whole_imagefile1) 
            whichpic = 1    
        if (event.widget == canvas2 ):
            img = dis_canvas.create_image(0.05*w_win, 0.45*h_win, anchor='w',image=whole_imagefile2)  
            whichpic = 2 
        if (event.widget == canvas3 ):
            img = dis_canvas.create_image(0.05*w_win, 0.45*h_win, anchor='w',image=whole_imagefile3) 
            whichpic = 3  
        if (event.widget == canvas4 ):
            img = dis_canvas.create_image(0.05*w_win, 0.45*h_win, anchor='w',image=whole_imagefile4)   
            whichpic = 4
        dis_canvas.place(x=0.05*w_win, y=0.5*h_win, anchor='w')
...
print(path)
pathh = tk.StringVar()
tk.Label(window,text = "目标路径:").place(x=0.01*w_win, y=0.025*h_win, anchor='w')
tk.Entry(window, textvariable = pathh).place(x=0.035*w_win, y=0.025*h_win,width = 300, anchor='w')
tk.Button(window, text = "路径选择", command = selectPath).place(x=0.21*w_win, y=0.025*h_win, anchor='w')
...
tk.Label(text='评分属性(请下拉选择)：').place(x=0.31*w_win, y=0.025*h_win, anchor='w')
cmb = ttk.Combobox(window)
cmb.place(x=0.365*w_win, y=0.025*h_win, anchor='w')
# 设置下拉菜单中的值
